...
> **This fork** is based on the original [`dbachelder/pi-btw`](https://github.com/dbachelder/pi-btw) repository and adds 4 things on top of the base version: a persistent `pi-btw.json` model/thinking default (`readBtwConfig`/`writeBtwConfig`), a follow-up queue so you can type the next question while one is still answering, `/btw resume` to pick and continue any earlier BTW thread from this session, and a scope picker (entire thread / last exchange / from exchange N) before `/btw inject` or `/btw summarize` instead of always sending the whole thread. See inline `ponytail:` comments in `extensions/btw.ts` for the simplifications made along the way.
...
```text
/btw ask what file defines this route?
/btw ask --save summarize the last error in one sentence
...
Lists every earlier BTW thread from the current session (by its first question, exchange count, and mode) and lets you pick one to continue. The current thread is excluded from the list.

### Persistent defaults

`/btw model` and `/btw thinking` also write to `pi-btw.json` in the pi agent dir, so the override becomes the default for brand-new sessions too, not just the current one. A session's own override (set/cleared via those commands) still wins over the file default for that session.

### Queued follow-ups

Type and submit another question while BTW is still answering the previous one — it queues (shown as "Queued (n pending)...") and runs right after, in order, instead of erroring.

### Scope picker on inject/summarize

When the thread has more than one exchange, `/btw inject` and `/btw summarize` now ask how much to bring to main: the entire thread, the last exchange only, or everything from a given exchange onward.

## Commands

### `/btw ask [--save] <question>`

`/btw <question>` remains a shorthand for `/btw ask <question>`, and `/btw` with no arguments opens the overlay.
...
[MIT](LICENSE) © Villoh. Forked from [`dbachelder/pi-btw`](https://github.com/dbachelder/pi-btw) © Dan Bachelder; see [LICENSE](LICENSE) for both notices.
