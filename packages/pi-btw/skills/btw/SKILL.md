...
BTW uses one parent command with subcommands:
...
`/btw <question>` is shorthand for `/btw ask <question>`. `/btw` with no arguments opens the overlay.
...
Use this when the user wants to pick and continue an earlier BTW thread from the current session.
...
- Prefer `/btw ask` over normal chat when the user explicitly wants a side conversation.
...
```text
/btw ask what are the risks of switching this to optimistic updates?
```
