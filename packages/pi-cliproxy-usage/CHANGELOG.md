...
- fce1401: Add `pi-extension`/`pi-skill` keywords so the pi.dev package gallery shows the correct EXTENSION/SKILL badge instead of the generic PACKAGE fallback.

## 0.4.1

### Patch Changes

- 9d77ff6: Show `/cliproxy-usage` usage and status in a dismissible TUI dialog with titled sections, themed labels, and compact day-based reset countdowns. Reuse fresh cached quota data for `/cliproxy-usage`; keep `/cliproxy-usage refresh` as a forced refresh.
